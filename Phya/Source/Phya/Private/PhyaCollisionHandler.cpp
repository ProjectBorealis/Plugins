...
static void InitPhya()
{
	if(GPhyaGlobals == NULL)
	{
		...
		paContact::pool.allocate(6 * N_BODIES);
		paImpact::pool.allocate(6 * N_BODIES);
		paFunSurface::contactGenPool.allocate(12 * N_BODIES); // Each Contact and Impact takes 2 contactGens.
		paFunSurface::impactGenPool.allocate(12 * N_BODIES); // Each Impact takes 2 impactGens.
...
void UPhyaCollisionHandler::InitCollisionHandler()
{
	...
	for(int32 i=0; i< N_BODIES; i++) 
	{
		...
		Res->setnActiveModes(10);		// Can trade detail for speed.
