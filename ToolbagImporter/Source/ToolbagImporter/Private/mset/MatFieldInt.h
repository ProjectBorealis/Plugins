...
class	MatFieldInt : public MatField
{
public:
	MatFieldInt(const char* name, const char* parameterName, UMaterial* Material) : MatField(name, parameterName, Material), mValue(0)
	{}
	void readValue( ToolbagUtils::MaterialSubroutine* subroutine ) override
	{
		ToolbagUtils::MatFieldInt* m = ToolbagUtils::GetMatFieldInt(subroutine, mName.c_str());
		if(m != 0)
			mValue = m->value;
	}
