...
{
#if WITH_EDITOR
	return FractureHistory.GetCurrentLoadedAsset().Get();
#else
	return Asset;
#endif
}

bool UBlastAsset::IsLoadedAssetValid() const
{
#if WITH_EDITOR
	return FractureHistory.GetCurrentLoadedAsset().IsValid();
#else
...
uint32 UBlastAsset::GetChunkCount()  const
{
	if (IsLoadedAssetValid())
	{
		return NvBlastAssetGetChunkCount(GetLoadedAsset(), Nv::Blast::logLL);
	}
	return 0;
}
...
uint32 UBlastAsset::GetBondCount()  const
{
	if (IsLoadedAssetValid())
	{
		return NvBlastAssetGetBondCount(GetLoadedAsset(), Nv::Blast::logLL);
	}
	return 0;
}
