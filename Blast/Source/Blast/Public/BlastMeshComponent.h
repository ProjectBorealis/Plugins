...
public:
	// If true, this actor will start simulating as a kinematic physics body
	UPROPERTY(VisibleAnywhere, Category = "Blast")
	bool							bIsInitiallyKinematic;

	UPROPERTY(EditAnywhere, Category = "Blast")
	bool							bSupportedByWorld;

	// If true, this component will start ignoring all kinds of incoming damage
	UPROPERTY(EditAnywhere, Category = "Blast")
	bool							bIgnoreDamage;

	// If a chunk body has smaller radius than this value, it will get small chunk body assigned instead
	UPROPERTY(EditAnywhere, Category = "Blast")
	float							SmallChunkRadius;

	// After an inmost chunk's health reaches to zero, we will hide them if this option is enabled
	UPROPERTY(EditAnywhere, Category = "Blast")
	bool							bCrumbleInmostChunks;
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = SmallChunkCollision, meta = (ShowOnlyInnerProperties, CantUseWithExtendedSupport))
	FBodyInstance SmallChunkBodyInstance;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = CrumbledChunkCollision, meta = (ShowOnlyInnerProperties, CantUseWithExtendedSupport))
	...
	* @param bRandomizeImpulse If true, we'll randomize impulses a bit
	* @return EBlastDamageResult Damage result enum. @see EBlastDamageResult
	*/
	UFUNCTION(BlueprintCallable, Category = "Blast")
	EBlastDamageResult ApplyRadialDamage(FVector Origin, float MinRadius, float MaxRadius, float Damage = 100.0f, float ImpulseStrength = 0.0f, bool bImpulseVelChange = true, bool bRandomizeImpulse = false);
	...
	*/
	...
	/**
	* Apply shear damage on this component. BlastRadialDamageProgram is used.
	*
	* @param Origin Damage origin
	* @param MinRadius Damage min radius. Damage is maximum inside of this radius.
	* @param MinRadius Damage max radius. Damage linearly falloffs to 0 from min to max radius.
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Blast")
	EBlastDamageResult ApplyShearDamage(FVector Origin, FQuat Direction, float MinRadius, float MaxRadius, float Damage = 100.0f, float ImpulseStrength = 0.0f, bool bImpulseVelChange = true);

	/**
	* Apply shear damage on all BlastMeshComponents inside of sphere overlap. BlastRadialDamageProgram is used.
	*
	* @param Origin Damage origin
	* @param MinRadius Damage min radius. Damage is maximum inside of this radius.
	* @param MinRadius Damage max radius. Damage linearly falloffs to 0 from min to max radius.
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Blast")
	static EBlastDamageResult ApplyShearDamageAll(FVector Origin, float MinRadius, float MaxRadius, float Damage = 100.0f, float ImpulseStrength = 0.0f, bool bImpulseVelChange = true);
	...
	*/
	...
	FBodyInstance* GetActorBodyInstance(int32 ActorIndex) const;
	...
	UFUNCTION(BlueprintCallable, Category = "Blast")
	void RestoreActorBodyInstanceObjectType(FName ActorName) const;
...
protected:
	...
	struct FActorData
	{
		...
		FActorData() : BlastActor(nullptr), BodyInstance(nullptr), bIsAttachedToComponent(false), bIsSmallChunk(false) {}
	};
