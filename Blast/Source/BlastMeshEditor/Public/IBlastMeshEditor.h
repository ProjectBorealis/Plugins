...
public:
	...
	/**
	Attempt to merge the chunks selected.  More than one chunk must be selected, and only chunks joined
	by bonds in the support graph may be merged.*/
	virtual void MergeSelectedChunks() = 0;
	...
	If bIncludeRoot == true, the root node is also removed.
	*/
	virtual void RemoveSubhierarchy(int32 ChunkId = INDEX_NONE, bool bIncludeRoot = false) = 0;
};
