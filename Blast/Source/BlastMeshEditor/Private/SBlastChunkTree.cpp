...
TSharedPtr<SWidget> SBlastChunkTree::ConstructContextMenu() const
{
	if (ChunkHierarchy.IsValid() && ChunkHierarchy->GetSelectedItems().Num())
	{
		...
		MenuBuilder.AddMenuEntry(	LOCTEXT("BlastChunkTree_mergeChunk", "Merge chunks"),
									LOCTEXT("BlastChunkTree_mergeChunkTT", "Merge of all selected chunks will be attempted, creating a new parent chunk"),
									FSlateIcon(),
									FExecuteAction::CreateSP(this, &SBlastChunkTree::MergeSelectedChunks));

		MenuBuilder.AddMenuEntry(	LOCTEXT("BlastChunkTree_removeChunk", "Remove chunk(s)"),
									LOCTEXT("BlastChunkTree_removeChunkTT", "All selected chunks and their children will be removed"),
									FSlateIcon(),
									FExecuteAction::CreateSP(this, &SBlastChunkTree::RemoveChunks));
