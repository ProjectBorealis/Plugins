...
public:
	...
	TSharedRef<SWidget> PopulateMenu()
	{
		...
		if (CurrentMesh)
		{
			for (const FSkeletalMaterial& SkelMat : CurrentMesh->Mesh->GetMaterials())
			{
...
{
	Setup(Other.Amplitude, Other.Frequency, Other.OctaveNumber, Other.SamplingInterval);
}
...
void UBlastFractureSettings::LoadDefault()
{
	...
	if (Config)
	{
		...
		UniformSlicingFracture->Setup(Config->NoiseAmplitude, Config->NoiseFrequency, Config->NoiseOctaveNumber, Config->NoiseSamplingInterval);
		
		CutoutFracture->Size = Config->CutoutSize;
		CutoutFracture->RotationZ = Config->CutoutRotationZ;
		CutoutFracture->Aperture = Config->CutoutAperture;
		CutoutFracture->bPeriodic = Config->bCutoutPeriodic;
		CutoutFracture->bFillGaps = Config->bCutoutFillGaps;
		CutoutFracture->Setup(Config->NoiseAmplitude, Config->NoiseFrequency, Config->NoiseOctaveNumber, Config->NoiseSamplingInterval);

		CutFracture->Setup(Config->NoiseAmplitude, Config->NoiseFrequency, Config->NoiseOctaveNumber, Config->NoiseSamplingInterval);
...
void UBlastFractureSettings::SaveAsDefault()
{
	...
	if (Config)
	{
		...
		Config->CutoutAperture = CutoutFracture->Aperture;
		...
		Config->NoiseSamplingInterval = UniformSlicingFracture->SamplingInterval;
