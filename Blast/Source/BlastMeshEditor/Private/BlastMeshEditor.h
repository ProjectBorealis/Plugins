...
public:
	...
	virtual void MergeSelectedChunks() override;
	virtual void RemoveSubhierarchy(int32 ChunkId = INDEX_NONE, bool bIncludeRoot = false) override;
...
private:
	...
	/** Callback to check if the last fracture operation can be undone. */
	bool CanUndoFracture();

	/** Callback for redo operation. */
	void RedoFracture();

	/** Callback to check if the next fracture operation can be redone. */
	bool CanRedoFracture();
