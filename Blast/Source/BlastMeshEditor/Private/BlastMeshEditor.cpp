...
void FBlastMeshEditor::BindCommands()
{
	...
	UICommandList->MapAction(
		Commands.Redo,
		FExecuteAction::CreateSP(this, &FBlastMeshEditor::RedoFracture),
		FCanExecuteAction::CreateSP(this, &FBlastMeshEditor::CanRedoFracture),
		FCanExecuteAction(),
		FIsActionChecked());
}
...
void FBlastMeshEditor::MergeSelectedChunks()
{
	if (SelectedChunkIndices.Num() < 2)
	{
		return;
	}
	Viewport->UpdatePreviewMesh(nullptr);
	TSet<int32> ChunkIndices = SelectedChunkIndices;
	Fracturer->BuildChunkHierarchy(FractureSettings, ChunkIndices, 1, ChunkIndices.Num(), false);
	SelectedChunkIndices.Empty();
	OnBlastMeshReloaded();
}

void FBlastMeshEditor::RemoveSubhierarchy(int32 ChunkId, bool bIncludeRoot)
{
	...
	Viewport->UpdatePreviewMesh(nullptr);
	Fracturer->RemoveSubhierarchy(FractureSettings, ChunkIndices, bIncludeRoot);
	SelectedChunkIndices.Empty();
	OnBlastMeshReloaded();
}
...
void FBlastMeshEditor::UndoFracture()
{
	Viewport->UpdatePreviewMesh(nullptr);
	Fracturer->Undo(FractureSettings);
	SelectedChunkIndices.Empty();
	OnBlastMeshReloaded();
}

bool FBlastMeshEditor::CanUndoFracture()
{
	return Fracturer->CanUndo(FractureSettings);
}

void FBlastMeshEditor::RedoFracture()
{
	Viewport->UpdatePreviewMesh(nullptr);
	Fracturer->Redo(FractureSettings);
	SelectedChunkIndices.Empty();
	OnBlastMeshReloaded();
}

bool FBlastMeshEditor::CanRedoFracture()
{
	return Fracturer->CanRedo(FractureSettings);
}
