...
class UMultiPackerProcessCore;
class FAssetEditorManager;
class SDockTab;

class FMultiPackerAssetEditor : public FAssetEditorToolkit, public FNotifyHook, public FGCObject
{
public:
	FMultiPackerAssetEditor();
	virtual ~FMultiPackerAssetEditor();

	void InitGenericGraphAssetEditor(const EToolkitMode::Type Mode, const TSharedPtr< IToolkitHost >& InitToolkitHost, UMultiPacker* Graph);
	...
	//Toolbar
	void UpdateToolbar();
	TSharedPtr<class FMultiPackerAssetEditorToolbar> GetToolbarBuilder() { return ToolbarBuilder; }
...
private:
	...
	UMultiPacker* EditingGraph;
	UMultiPackerProcessCore* MPCore;
	bool NodesChanged = true;
	bool canProcess = true;
	//Toolbar
	TSharedPtr<class FMultiPackerAssetEditorToolbar> ToolbarBuilder;
